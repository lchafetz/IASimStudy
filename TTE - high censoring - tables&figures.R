###########################################################################
#                                                                         #
# Script for TTE outcome simulations (high censoring)  - TABLES AND PLOTS #
#                                                                         #
###########################################################################
rm(list=ls())
...
#HIGH CENSORING
load("results.high.over.RData")
load("results.high.well.RData")
load("results.high.under.RData")

c1<-results.high.under
c2<-results.high.well
c3<-results.high.over
...
ggsave(power,file="power.tte.high.png", height = 5, width = 8.889,dpi=400)
...
ggsave(ss,file="ss.tte.high.png", height = 5, width = 8.889,dpi=400)

figure <- ggarrange(power, ss,
                    labels = c("(a)", "(b)"),
                    ncol = 1, nrow = 2)

ggsave(figure,file="tte.plots.high.png", height = 10, width = 8.889,dpi=400)
