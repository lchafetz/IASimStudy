...
#   Low censoring, control hazard underestimated  #
###################################################
...
n=360 #sample size for assumed beta_0 of -1 (change sample size depending on phi and assumed beta_0)
...
for(p in 1:4){
  for(i in 1:nsim){
    mod.half.E[[p]][[i]]<-stan_surv(Surv(Surv_times_half, event) ~ trt, data = data.halfE[[p]][[i]],
                                    basehaz = "weibull", chains = 2, seed = 448290,iter=2000)
  }}

save(mod.half.E, file="E.half.low.under.RData")
...
save(mod.half.SS, file="SS.half.low.under.RData")
...
for(p in 1:4){
  for(i in 1:nsim){
    mod.final.E[[p]][[i]]<-stan_surv(Surv(Surv_times_final, event) ~ trt, data = data.final[[p]][[i]], 
                                          basehaz = "weibull", chains = 2, seed = 8849030,
                                          prior=normal(summary.mod.half.E[[p]][[i]]$mean[2],summary.mod.half.E[[p]][[i]]$sd[2]),iter=2000)
  }}

save(mod.final.E, file="E.final.low.under.RData")
...
save(mod.final.SS, file="SS.final.low.under.RData")
...
results.low.under<-data.frame(rbind(r1,r2,r3,r4,r5,r6,r7,r8))
rownames(results.low.under)<-c("P1","T1","D1","N1","P2","T2","D2","N2")

save(results,file="results.low.under.RData")
results.low.under
