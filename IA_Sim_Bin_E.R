...
n<-2130 # sample size for assumed p_0=0.10 (change to 962 for assumed p_0=0.20 or change to 574 for assumed p_0=0.30)
D<-182 # number of expected events for assumed p_0=0.10 (change to 166 for assumed p_0=0.20 or change to 148 for assumed p_0=0.30)
...
for(p in 1:4){
  for(i in 1:Nsim){
    nc[[p]][i]<-min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==halfwayE),"cum_ctrl_pts"])  
    dc[[p]][i]<-min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==halfwayE),"cum_ctrl_events"]) 
...
for(p in 1:4){
  for(i in 1:Nsim){
    nt[[p]][i]<-min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==halfwayE),"cum_trt_pts"])
    dt[[p]][i]<-min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==halfwayE), "cum_trt_events"])
...
avg_sample_size<-c()
for(p in 1:4){
  avg_sample_size[p]<-ceiling(mean(unlist(nc[[p]]))+mean(unlist(nt[[p]])))
}
...
for(p in 1:4){
  for(i in 1:Nsim){
    if (postprob[[p]][[i]]<sup){
      nc_final[[p]][[i]]<-ifelse(max_events[[p]][[i]]<D,
                                 max(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==max_events[[p]][[i]]),"cum_ctrl_pts"]),
                                 min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==D),"cum_ctrl_pts"]));
      
      dc_final[[p]][[i]]<-ifelse(max_events[[p]][[i]]<D, 
                                 max(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==max_events[[p]][[i]]),"cum_ctrl_events"]),
                                 min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==D),"cum_ctrl_events"]));
...
for(p in 1:4){
  for(i in 1:Nsim){
    if (postprob[[p]][[i]]<sup){
      nt_final[[p]][[i]]<-ifelse(max_events[[p]][[i]]<D,
                                 max(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==max_events[[p]][[i]]),"cum_trt_pts"]),
                                 min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==D),"cum_trt_pts"]));
      dt_final[[p]][[i]]<-ifelse(max_events[[p]][[i]]<D, 
                                 max(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==max_events[[p]][[i]]),"cum_trt_events"]),
                                 min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_events==D),"cum_trt_events"]));
...
avg_d<-c()
for(p in 1:4){
  avg_d[p]<-ceiling(mean(na.omit(unlist(dc_final[[p]])))+mean(na.omit(unlist(dt_final[[p]]))))
}
...
r5<-ceiling(avg_n_overall) # expected sample size overall (N_2)
r6<-ceiling(avg_d_overall) # expected number of events overall (D_2)


results<-data.frame(rbind(r1,r2,r3,r4,r5,r6))
rownames(results)<-c("P_1","N_1","D_1","P_2","N_2","D_2")
results
