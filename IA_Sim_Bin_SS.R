...
for(p in 1:4){
  for(i in 1:Nsim){
    nc[[p]][i]<-ceiling(halfwaySS/2)
    dc[[p]][i]<-min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_ctrl_pts==ceiling(halfwaySS/2)),"cum_ctrl_events"])
...
for(p in 1:4){
  for(i in 1:Nsim){
    nt[[p]][i]<-ceiling(halfwaySS/2)
    dt[[p]][i]<-min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_trt_pts==ceiling(halfwaySS/2)),"cum_trt_events"])
...
for(p in 1:4){
  for(i in 1:Nsim){
    if (postprob[[p]][i]<sup){
      nc_final[[p]][i]<-n/2;
      dc_final[[p]][i]<-min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_ctrl_pts==ceiling(n/2)),"cum_ctrl_events"]);
...
for(p in 1:4){
  for(i in 1:Nsim){
    if (postprob[[p]][i]<sup){
      nt_final[[p]][i]<-n/2;dt_final[[p]][i]<-min(dataset[[p]][[i]][which(dataset[[p]][[i]]$cum_trt_pts==ceiling(n/2)),"cum_trt_events"]); 
...
r3<-ceiling(avg_d) # expected number of events halfway  (D_1)
